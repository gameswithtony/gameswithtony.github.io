...
- **The block's total mine count is stated at placement time** ("this generation introduced 3 defects"). Required, not flavor. Without a per-block count, deduction on an irregular island is frequently unsolvable. The player is choosing where to put a known quantity of defects.
- **If no legal placement exists anywhere, the turn is refunded and no block is given.**
